...
namespace 雪莉轻食
{
    public partial class Quren : Form
    {
        public Quren()
        {
            InitializeComponent();
        ...
        private void Form9_Load(object sender, EventArgs e)
        {
            ...
            int a = 0;
            int b = 0;
            DataTable objDataTable = DBHelper.GetDataTable("select * from  TheOrderTable a inner join OrderStateTable b on a.OrderNumber=b.OrderNumber where b.OrderState='等待确认'");
        ...
        private void Button_Click(object sender, EventArgs e)
        {
            ...
            PP = int.Parse(b2.Name.ToString());
            DataTable objDataTable2 = DBHelper.GetDataTable("select * from  TheOrderTable where OrderID="+PP+"");
          
         
            if (DBHelper.SaveData("update OrderStateTable set OrderState='等待派送'where OrderNumber='" + objDataTable2.Rows[0]["OrderNumber"].ToString() + "'") > 0)
            {
                MessageBox.Show("派送确认成功", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                Main_Form.GTY.button6_Click(null,null);
            }
            else
            {
                MessageBox.Show("派送确认失败", "提示", MessageBoxButtons.OK, MessageBoxIcon.Stop);
            }
