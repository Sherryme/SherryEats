...
namespace 雪莉轻食
{
    public partial class PaiWanChen : Form
    {
        public PaiWanChen()
        {
            InitializeComponent();
        ...
        private void Form10_Load(object sender, EventArgs e)
        {
            ...
            int a = 0;
            int b = 0;
            DataTable objDataTable = DBHelper.GetDataTable("select * from  TheOrderTable a inner join OrderStateTable b on a.OrderNumber=b.OrderNumber where b.OrderState='派送中'");
        ...
        private void Button_Click(object sender, EventArgs e)
        {
            ...
            FF = int.Parse(b2.Name.ToString());
            DataTable objDataTable2 = DBHelper.GetDataTable("select * from  TheOrderTable where OrderID=" + FF + "");


            if (DBHelper.SaveData("update OrderStateTable set OrderState='派送完成'where OrderNumber='" + objDataTable2.Rows[0]["OrderNumber"].ToString() + "'") > 0)
            {
                MessageBox.Show("确认派送完成成功", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                Main_Form.GTY.button4_Click(null, null);
            }
            else
            {
                MessageBox.Show("确认派送完成失败", "提示", MessageBoxButtons.OK, MessageBoxIcon.Stop);
            }
