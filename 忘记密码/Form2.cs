...
namespace 雪莉轻食
{
    public partial class Form2 : CCSkinMain
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                string strSQL = "select * from UserTable where UserName='" + this.textBox1.Text.ToString().Trim() + "'";
                Form5 aa = new Form5();
                DataSet da = aa.GetDataSet(strSQL);
                if (da.Tables.Count == 0 || da.Tables[0].Rows.Count == 0)
                {
                    MessageBox.Show("不存在该姓名", "错误", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                    return;
                }
            }
            if (textBox2.Text.ToString().Trim().ToUpper() != skinCode1.CodeStr.ToString())
            {
